...
function SearchBar({ coursesData }: { coursesData?: Array<any> }) {
  ...
  const coursesDataFiltered = coursesData?.filter((course: any) =>
    isSearchResult(course, search)
  );
    ...
    <div className={styles.searchContainer}>
      ...
      {search != "" &&
        coursesDataFiltered?.map((course: any) => (
          <Link
            key={`LILSEARCHRESULT${course["id"]}`}
            href={`${course["id"]}`}
            className={styles.searchResults}
          >
            <b>{course["Course Number"]}</b>: {course["Course Name"]}
          </Link>
        ))}
    </div>
  );
  ...
  return (
    <header className={styles.headerComponent}>
      <div className={styles.header}>
        <nav>
          <h1>
            <Link href="/">(old)Syllabuddies</Link>
          </h1>
